...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Kazucoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f, 0x65c6419f, 0x8db6448a
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Kazucoin is probably already running."), addrBind.ToString().c_str());
        else
