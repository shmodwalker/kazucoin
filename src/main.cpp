...
uint256 hashGenesisBlock("0x758f3d45d6b5d2b08b3715c4fc1a75b6dc46ee5c08f059ccd2ea14e1c50f6349");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kazucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kazucoin Signed Message:\n";
...
{
    // Kazucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 105000); // Kazucoin: 105k blocks 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Kazucoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Kazucoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kazucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x002a7836ad62cfc71666fc678674e07af906d4dfce9679865ca7a29dd0a16ecc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1526880792;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086385092;

        if (fTestNet)
        {
            block.nTime    = 1526880772;
            block.nNonce   = 386278538;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x497b3d800d6fb1e12b6ffd3503ca43d1379a482ddc3a6a8c5e78094a99532017"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc6, 0xa8, 0xbf, 0xd4 }; // Kazucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KazucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KazucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KazucoinMiner(CWallet *pwallet)
{
    printf("KazucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kazucoin-miner");
    ...
    try { loop {
        ...
        printf("Running KazucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KazucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KazucoinMiner, pwallet));
}
